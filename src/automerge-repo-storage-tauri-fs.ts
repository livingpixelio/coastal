// https://gitlab.com/jcc333/automerge-repo-storage-tauri-fs/-/raw/main/packages/automerge-repo-storage-taurifs/src/index.ts?ref_type=heads
...
export class TauriFileSystemStorageAdapter extends StorageAdapter {
  ...
  private resolvedBaseDirectory: string | null = null;
  private cache: { [key: string]: Uint8Array } = {};

  constructor(baseDirectory: string = "automerge-repo-data") {
    super();
    this.baseDirectoryName = baseDirectory;
  }

  private async getBaseDirectory(): Promise<string> {
    if (!this.resolvedBaseDirectory) {
      const appLocalData = await path.appLocalDataDir();
      this.resolvedBaseDirectory = await path.join(appLocalData, this.baseDirectoryName);
    }
    return this.resolvedBaseDirectory;
  }

  async load(storageKey: StorageKey): Promise<undefined | Uint8Array> {
    let result = await this.loadCache(storageKey);
    if (!result) {
      result = await this.loadFile(storageKey);
      if (result) {
  ...
  async loadRange(keyPrefix: StorageKey): Promise<Chunk[]> {
    ...
    const diskKeys: string[] = await Promise.all(
      diskFiles.map(async (fileName: string) => {
        const relativePath = await relative(baseDir, fileName);
  ...
  private async saveFile(
    ...
    await fs.mkdir(filePathDir, { recursive: true });
  ...
  private async removeRangeCache(keyPrefix: StorageKey): Promise<void> {
    return this.cachedKeys(keyPrefix).then((keys) =>
...
const getKey = async (key: StorageKey): Promise<string> =>
  await path.join(...key);
